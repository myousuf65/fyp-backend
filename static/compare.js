	...
	*/
	...
	imgcanvas.toBlob(async(blob) => {
		if (blob instanceof Blob) {
			...
			// check if an actual person is in the image
			...
			if(detections.length === 0){
				showModal("Could not detect a human in the image", 500);
				return;
			}
			...
			fetch(`${window.location.origin}/match/compareface/`, {
				...
				.then(data => {
					...
					if (data["message"] === "True") {
						showModal(`Face Matched with ${data.matched_person_names[0]}`, 200)
					}
...
video.addEventListener('playing', () => {
	...
	setInterval(async () => {
		...
		// detect all faces from the video input stream
		const detections = await faceapi.detectAllFaces(
			video,
			new faceapi.TinyFaceDetectorOptions()
		).withFaceLandmarks();

		if (detections.length === 0) { console.log("no face detected") }
